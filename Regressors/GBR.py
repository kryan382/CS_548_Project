# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 18:11:44 2024
...
rows = num_depths * num_trees * num_rate
...
for lr_idx, lrate in enumerate(l_rate):
    ...
    for depth_idx, depth in enumerate(range(1, num_depths + 1)):
        for tree_idx, n_trees in enumerate(trees):
            gbr = GradientBoostingRegressor(max_depth=depth,random_state=17,n_estimators=n_trees,learning_rate=lrate)
            ...
            cv_scores = RepeatedKFold(n_splits=5, n_repeats=3, random_state=8)
            Accuracy_Values2 = cross_val_score(gbr, X_train, y_train, cv=cv_scores, scoring=custom_Scoring2)
...
#MAE
df_metricsMAE = pd.DataFrame(all_cv_scores2, columns=[f'Fold {i+1}' for i in range(cv_folds)])
# # A20
df_metricsa20 = pd.DataFrame(all_cv_scores3, columns=[f'Fold {i+1}' for i in range(cv_folds)])
# 
file_path = "C:/spydertest/csv/CumulRot.xlsx"

with pd.ExcelWriter(file_path, mode='a', engine='openpyxl') as writer:
    df_metrics.to_excel(writer, sheet_name='GBRtrainmapeMOVIE', index_label='Row Index')
    df_metricsMAE.to_excel(writer, sheet_name='GBRtrainMAEMOVIe', index_label='Row Index')
    df_metricsa20.to_excel(writer, sheet_name='GBRtraina20MOVIE', index_label='Row Index')